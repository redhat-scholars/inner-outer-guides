...
You'll need to authorize the Argo CD application to access your OpenShift cluster. Click on the *Allow selected permissions* button to authorize Argo CD to access your OpenShift cluster. 
...
You'll also have access to multiple *Gitea* repositories that contain the Vote App source code and Argo CD application manifests. You can access Gitea from the *Lab Guides*, or you can visit Gitea using this link:https://gitea.%SUBDOMAIN%[*Gitea Link*,role='params-link',window='_blank']. As a reference, here are your credentials:
