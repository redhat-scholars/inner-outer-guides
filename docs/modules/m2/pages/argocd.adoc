...
Now, let's use the web terminal to modify a deployment and demonstrate that we have not yet put in place self-healing or high availability just yet. From the OpenShift Web Console in the `vote-app-dev-%USERID%` project, click on the *Terminal* icon from the top right of the page.
